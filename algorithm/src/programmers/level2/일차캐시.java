...
public class 일차캐시 {
    ...
    public static void main(String[] args) {
        ...
        solution(0, new String[]{"Jeju", "Pangyo", "NewYork", "newyork", "LA"});
    ...
    private static int solution(int cacheSize, String[] cities) {
        ...
        for (String cite : cities) {
            String citeLow = cite.toLowerCase();
            if (queue.contains(citeLow)) { // Queue 가 이미 존재하다면, 캐시가 존재하다면 cache hit
                queue.remove(citeLow); // 지정해서 삭제도 가능하다 Stack 도 가능
                queue.offer(citeLow);
                System.out.println("Hit queue = " + queue);
                answer++;
            } else { // 존재하지 않다면 cache miss
                if (queue.size() < cacheSize) {
                    queue.offer(citeLow); // cite queue 에 넣기
                } else {
                    queue.poll(); // 가장 먼저 들어온 (캐시가 가장 오래된) cite 를 삭제
                    queue.offer(citeLow); //
                }
                System.out.println("Miss queue = " + queue);
                answer += 5;
            }
        }
