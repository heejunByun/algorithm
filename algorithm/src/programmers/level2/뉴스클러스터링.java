...
class NewsCluster {
    public int solution(String str1 , String str2) {
        ...
        //모든 글자들을 합집합 영역에 저장
        //교집합 나누기 합집합 값을 곱하기 65536 을 한 후 소수점 버리고 정수만 출력
        ...
        for (int i = 1; i < str1.length(); i++) {
            ...
            if (check.matches("^[a-zA-Z]*$")) // 영문만 들어오는 정규식 ^문자열의 시작, $ 문자열의 종료
                strList1.add(check);
        }
        for (int i = 1; i < str2.length(); i++) {
            String check = String.valueOf(str2.charAt(i - 1)) + String.valueOf(str2.charAt(i));
            if (check.matches("^[a-zA-Z]*$")) // 영문만 들어오는 정규식 ^문자열의 시작, $ 문자열의 종료
                strList2.add(check);
        }
